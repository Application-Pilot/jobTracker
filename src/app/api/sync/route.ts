...
async function upsertApplications(apps: any[]) {
  if (apps.length === 0) {
...
async function runSync() {
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    console.log((i + 1) + '/' + todo.length + ' ' + (todo[i] as any).id);
    const msg = await getMessageBody(gmail, (todo[i] as any).id);
    if (!msg) continue;
